...
class nav_bits
{
  ...
  {
    global $diy_db;
    $result = $diy_db->query("SELECT catid,parent,cat_title FROM diy_download_cat ORDER BY catid ASC");


    while ($row = $diy_db->dbarray($result))
    {
      extract($row);
      $Master[] = array('id' => $catid, 'subcat' => $parent, 'title' => $cat_title);
    }
    ...
    {
      $Last .= '<a href="mod.php?mod=download&modfile=list&catid=' . $NewArray[$i]['id'] . '"> ' . $NewArray[$i]['title'] . ' </a > » ';
    }

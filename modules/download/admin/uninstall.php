...
if (($_GET['step'] == '0') || ($_GET['step'] == ''))
{
  ...
  $form_array = array("action" => "sections.php?section=modules&file=uninstall&module=$module&&modid=$id&step=1&" . $auth->get_sess(), "title" => $admin_lang['UNINSTALL_MOUDLE'],
    "name" => 'settings', "content" => $form, "submit" => LANG_YES, );
...
} elseif ($_GET['step'] == '1')
{
  ...
  {
    $modid = $_GET[modid];
    $diy_db->query("DROP TABLE IF EXISTS `diy_download_files`;");
    $diy_db->query("DROP TABLE IF EXISTS `diy_download_cat`;");
    $diy_db->query("DROP TABLE IF EXISTS `diy_download_comments`;");

...
if ($action == "" )
{
	 ...
	 $search_form .= $form->hiddenform("mod","news");
     ...
     $form_array = array(
         "action" => "mod.php?mod=news&modfile=search&action=do_search",
		 "method" => 'post',
...
else if($_GET['action']=="do_search")
{
 ...
 if (!isset($_GET['start'])) {
     ...
     $result = $diy_db->query("SELECT * FROM diy_news
							 WHERE allow = 'yes'
     ...
     while ($row = $diy_db->dbarray($result)) {
         ...
         $date    = format_date($date_added);
             ...
             eval("\$search_row .= \" " . $mod->gettemplate('news_search_results_row') . "\";");
         }
     eval("\$index_middle .= \" " . $mod->gettemplate('news_search_results') . "\";");
	 
	 $numrowsa = $diy_db->dbnumquery("diy_news", "allow = 'yes' AND (post LIKE '%$keyword%' OR title LIKE '%$keyword%')");
 $index_middle .= pagination($numrowsa, $posts_per_page, "mod.php?mod=news&modfile=search&action=do_search&keyword=$keyword&orderby=$order_by&sortby=$sort_by&pages=$posts_per_page");
   echo $index_middle;
 }

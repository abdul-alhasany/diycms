...
if (RUN_SECTION !== true)
{
 ...
 $query   = array();
 $query[] = "DROP TABLE IF EXISTS `diy_news_cat`;";
 $query[] = "
CREATE TABLE IF NOT EXISTS `diy_news_cat` (
 ...
 $query[] = "DROP TABLE IF EXISTS `diy_news`;";
 $query[] = "
CREATE TABLE IF NOT EXISTS `diy_news` (
 ...
 $query[] = "DROP TABLE IF EXISTS `diy_news_comments`;";
 $query[] = "CREATE TABLE IF NOT EXISTS `diy_news_comments` (
 ...
 $query[] = "INSERT INTO `diy_news_cat` (`catid`, `cat_order`, `parent`, `cat_title`, `dsc`, `dscin`, `countopic`, `countcomm`, `lastpostid`, `grouppost`, `groupview`, `cat_email`, `closed`) VALUES
(1, 0, 0, 'Test cateogry', 'This category is for testing purposes only', '', 1, 0, 1, '1,2,3,4,5,9', '1,2,3,4,5,9', '', '0');";

  $query[] = "INSERT INTO `diy_news` (`newsid`, `userid`, `cat_id`, `username`, `title`, `date_added`, `post`, `allow`, `readers`, `comments_no`, `status`, `rating_total`, `ratings`, `lastuserid`, `uploadfile`, `edit_by`) VALUES
(1, 1, 1, 'admin', 'Test post', 1249996876, '[b]This post is for testing purposes only[/b]. [i]You can edit or delete this post.[/i]', 'yes', 1, 0, '0', 0, 0, 0, 0, '0');";
 ...
 $query[] = "INSERT INTO diy_modules VALUES ('', '$module', '$admin_lang[mod_title]', '$admin_lang[mod_user]', 1, '$admin_lang[left_menu]','$admin_lang[right_menu]','0', '$admin_lang[menuid],$blockid',1,'english');";
 ...
 if (!$false == true) {
  ...
  $diy_db->query("INSERT INTO diy_menu VALUES ('', 'news control', 'standard_menu', 'mainmenu', ' Õﬂ„ «·√Œ»«—', '<!--INC dir=\"modules/news/blocks\" file=\"control.block.php\" -->', '2','1','$modid', '0', '1');");

  ...
  */
   ...
   $catid = set_id_int('catid');
	...
	if ($_POST['submit']) {
        ...
        upload_image("cat_image", "200", "200", "$catid.newscat");
        
        $result = $diy_db->query("update diy_news_cat set cat_order = '$order',
                                                    parent='$parent',
                                                    cat_title= '$title',
                                                    dsc= '$dsc',
													grouppost= '$allow_post',
                                                    groupview='$allow_view',
                                                    cat_email= '$email',
                                                    closed='$closed'
													where catid='$catid'
													");
        ...
        if ($result) {
             info_message($lang['CONTROL_EDITCAT_SUCCESSFUL'], 'mod.php?mod=news&dir=control&modfile=viewcat');
        } else {
        ...
        $catresult    = $diy_db->query("SELECT * FROM diy_news_cat ORDER BY catid");
        $cat_array[0] = 'Main';
        ...
        $result = $diy_db->query("SELECT * FROM diy_news_cat WHERE catid='$catid'");
        ...
        if (file_exists($avatarfile)) {
             $catimage = "<img src=filemanager.php?action=getimage&info=$catid;newscat;news>";
        } else {
		...
		<br><a href=mod.php?mod=news&dir=control&modfile=misc&action=delete_cat_image&catid=$catid>Delete current image</a></td>
        ...
        $form_array = array(
             "action" => "mod.php?mod=news&dir=control&modfile=edit_cat&catid=$catid",

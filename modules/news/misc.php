...
if ($_GET['action'] == "attachment") {
	...
	if ($diy_db->dbnumrows($result) > 0) {
		while ($rowfile = $diy_db->dbarray($result)) {
			extract($rowfile);
				$pathfile = get_file_path("$upid.news");
...
elseif ($_GET['action'] == "change_status") {
	...
	{
		case 'pin_topic':
		$diy_db->query("UPDATE diy_news SET status='0' WHERE newsid ='$newsid'");
		break;
		
		case 'unpin_topic':
		$diy_db->query("UPDATE diy_news SET status='1' WHERE newsid ='$newsid'");
		break;
		
		case 'close_topic':
		$diy_db->query("UPDATE diy_news SET closed='1' WHERE newsid ='$newsid'");
		break;
	...
	info_message($lang['MISC_ACTIONS_PERFORMED_SUCCESSFLY'], "mod.php?mod=news&modfile=viewpost&newsid=$newsid");
}

  ...
  include("modules/" . $mod->module . "/settings.php");
   ...
   $catid = set_id_int('catid');
    ...
    if ($_POST['submit']) {
        ...
        $result = $diy_db->query("update diy_forum_cat set cat_order = '$order',
                                                    parent='$parent',
                                                    cat_title= '$title',
                                                    dsc= '$dsc',
        ...
        if ($result) {
             info_message($lang['CONTROL_EDITCAT_SUCCESSFUL'], 'mod.php?mod=forum&dir=control&modfile=viewcat');
        } else {
        ...
        $catresult    = $diy_db->query("SELECT * FROM diy_forum_cat ORDER BY catid");
        $cat_array[0] = 'Main';
        ...
        $result = $diy_db->query("SELECT * FROM diy_forum_cat WHERE catid='$catid'");
        ...
        if (file_exists($avatarfile)) {
             $catimage = "<img src=filemanager.php?action=getimage&info=$catid;forumcat;forum>";
        } else {
		...
		<br><a href=mod.php?mod=forum&dir=control&modfile=misc&action=delete_cat_image&catid=$catid>Delete current image</a></td>
        ...
        $form_array = array(
             "action" => "mod.php?mod=forum&dir=control&modfile=edit_cat&catid=$catid",
